...
namespace Examples
{
    // Class declaration of a flat struct
    public struct FlatStruct
    {
        ...
        // ctor of a flat struct
        public FlatStruct(byte aa, short bb, float cc)
        {
            a = aa;
            b = bb;
            c = cc;
        ...
        // Call a method on a flat struct
        public float Method()
        { 
            // Read access from within method
            float ret = a + b + b;
            
            // Write access from within method
            a = (byte)(c - b);
            return ret;
        }
    ...
    public class Simple : RenderCanvas 
    {
        ...
        {
            /////////////////////////
            // Flat Struct use cases
            /////////////////////////
            
            // Variable declaration & Constructor call
            int nStructs = 1000000;
            FlatStruct [] flatStructs = new FlatStruct[nStructs];

            double t1 = Diagnostics.Timer;
            for (int i = 0; i < nStructs; i++)
            {
                flatStructs[i] = new FlatStruct((byte) i, (short) (i+1), i*3.1415f);
            }
            double t2 = Diagnostics.Timer;
            Diagnostics.Log("Initializing " + nStructs + " FlatStruct objects (classic way) took " + (t2 - t1) + " ms.");

            float result=0;
            double t3 = Diagnostics.Timer;
            for (int i = 0; i < nStructs; i++)
            {
                result += flatStructs[i].Method();
            }
            double t4 = Diagnostics.Timer;
            Diagnostics.Log("Calling Method on " + nStructs + " FlatStruct objects (classic way) took " + (t4 - t3) + " ms.");

            // Read access
            float f4 = flatStructs[2].a;
